...
syscall_handler (struct intr_frame *f UNUSED) 
{
  ...
  check_address((void *)sp,f->esp);               // check address of esp
...
//printf("in syscall.c %d\n", syscall_n);
  switch(syscall_n)
  {
	...
	//	check_address((void *)arg[0]);          // check string pointer
		check_valid_string((const void *)arg[0],sp);
	...
	if(v_entry == NULL )
		{
			...
			if(addr >= esp -32 ) // over original stack size -> more stack page
			{
		...
		if(v_entry->is_loaded != true)
			if(!handle_mm_fault(v_entry))exit(-1);
		return v_entry;
	}
	 return v_entry;                         // return ( void -> vm_entry* )
			 ...
			 bool to_write)
{
	...
	for(i = 0; i < size; i++) // for all buffer space
	{
		...
		struct vm_entry *v_temp = check_address((void *)temp_buffer,esp);
		...
		//v_temp = check_address(temp_buffer,esp);
	}	
}
